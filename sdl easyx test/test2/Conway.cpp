...
//////////////////////////////////////////////////////////////////////
//以下为测试的图形处理界面
void DrawGrid(Conway *c) {
    // 初始化图形窗口，假设每个格子是 10x10 像素
    initgraph(c->rows * 10, c->cols * 10,1);

    for (int i = 0; i < c->rows; ++i) {
        for (int j = 0; j < c->cols; ++j) {
            if (c->special_grids[i][j].type == living_producer) {
                setfillcolor(GREEN); // 设置填充色为蓝色
            }else if(c->special_grids[i][j].type == living_producer){ 
                setfillcolor(RED);
            } else {
            ...
            // 绘制矩形以表示单元格
            fillrectangle(j * 10, i * 10, (j + 1) * 10 - 1, (i + 1) * 10 - 1);
        }
    }
