...
Conway *update_conway(uint16_t rows, uint16_t cols) {
    ...
    c->normal_grids = (GridState **)malloc(c->rows * sizeof(GridState *));
    if (c->normal_grids == NULL) {
        safeFree(c);
        return NULL;
    }
    for (int i = 0; i < rows; i++) {
        c->normal_grids[i] = (GridState *)malloc(c->cols * sizeof(GridState));
        if (c->normal_grids[i] == NULL) {
            for (int j = 0; j < rows; j++) {
                free(c->normal_grids[j]);
            }
            safeFree(c->normal_grids);
            safeFree(c);
            return NULL;
        }
...
Conway *new_conway_special(uint16_t rows, uint16_t cols) {
    ...
    c->special_grids = (cell **)malloc(c->rows * sizeof(cell *));
    if (!c->special_grids) {
        free(c);
        return NULL;
    }
    for (int i = 0; i < c->rows; i++) {
        c->special_grids[i] = (cell *)malloc(c->cols * sizeof(cell));
    ...
    c->food_grids = (food_gridstate **)malloc(c->rows * sizeof(food_gridstate *));
    if (!c->food_grids) {
        free(c);
        return NULL;
    }
    for (int i = 0; i < c->rows; i++) {
        c->food_grids[i] = (food_gridstate *)malloc(c->cols * sizeof(food_gridstate));
        if (!c->food_grids[i]) {
            for (int j = 0; j < c->rows; j++) {
                free(c->food_grids[j]);
            }
