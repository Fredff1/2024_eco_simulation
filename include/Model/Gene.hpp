...
public:
    Gene(){
        ...
        oxygenGene = RandomUtil::getRandomInt(
            mapRangeToGene(midOxygen - 10, MIN_OXY_RANGE, OXY_RANGE),
            mapRangeToGene(midOxygen + 10, MIN_OXY_RANGE, OXY_RANGE)
        );
        ...
        pHGene = RandomUtil::getRandomInt(
            mapRangeToGene(midPH - 3, MIN_PH_RANGE, PH_RANGE),
            mapRangeToGene(midPH + 3, MIN_PH_RANGE, PH_RANGE)
        );
        geneMap[PH_GENE]=pHGene;
        float midAge=5000;
        ageGene= RandomUtil::getRandomInt(
            mapRangeToGene(midAge-5000,MIN_AGE_RANGE,AGE_RANGE),
            mapRangeToGene(midAge+5000,MIN_AGE_RANGE,AGE_RANGE)
        );
        geneMap[AGE_GENE]=ageGene;
        float midHUnger=500;
        hungerGene=RandomUtil::getRandomInt(
            mapRangeToGene(midHUnger-300,MIN_HUNGER_RANGE,HUNGER_RANGE),
            mapRangeToGene(midHUnger+300,MIN_HUNGER_RANGE,HUNGER_RANGE)
        );
        geneMap[HUNGER_GENE]=hungerGene;
        float midHealth=40000;
        healthGene=RandomUtil::getRandomInt(
            mapRangeToGene(midHealth-20000,MIN_HEALTH_RANGE,HEALTH_RANGE),
            mapRangeToGene(midHealth+20000,MIN_HEALTH_RANGE,HEALTH_RANGE)
        );
