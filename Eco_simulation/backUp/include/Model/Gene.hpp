...
public:
    ...
    float calculateActualTemperature(float actualTemperature){
        float optimalTemperature = mapGeneToRange(temperatureGene, MIN_TEMPERATURE_RANGE, TEMPERATURE_RANGE, 0);
        return  calculateFitness(actualTemperature, optimalTemperature, 20, 0.05);
    }
    /* Return a fitness score */
    float calculateActualOxygen(float actualOxygen){
        float optimalOxygen = mapGeneToRange(oxygenGene, MIN_OXY_RANGE, OXY_RANGE, 0);
        return calculateFitness(actualOxygen, optimalOxygen, 5, 0.1);
    }

    /* Return a fitness score */
    float calculateActualPH(float actualPH){
        float optimalPH = mapGeneToRange(pHGene, MIN_PH_RANGE, PH_RANGE, 0);
        return calculateFitness(actualPH, optimalPH, 3, 0.15);
    }

    /* Return the max age according to gene*/
    float calculateActualMaxAge(){
        return mapGeneToRange(ageGene,MIN_AGE_RANGE,AGE_RANGE,0);
    }
    /* Return the max hunger according to gene*/
    float calculateActualMaxHunger(){
        return mapGeneToRange(hungerGene,MIN_HUNGER_RANGE,HUNGER_RANGE,0);
    }

    float calculateActualMaxHealth(){
        return mapGeneToRange(healthGene,MIN_HEALTH_RANGE,HEALTH_RANGE,0);
    }
